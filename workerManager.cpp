...
workerManager ::~workerManager()    // 析构函数，释放内存
{
...
void workerManager ::Add_Emp() // 添加职工
{
    ...
    if (add_num > 0)    // 防止误输入
    {
        ...
        // 2、照搬旧数据，然后存入新职工的数据
        if (this->EmpArray != NULL)     // 如果当前职工数组为空，则会直接存入新的数据
        {
            for (int i = 0; i < this->EmpNum; i++)
            {
...
void workerManager ::save() // 在添加或删除职工操作后，保存到文件
{
    ofstream ofs;
    // 写方式打开文件
    ofs.open(FILE, ios::out);
    // 把员工数组中的员工数据存到文件中，各项成员用空格隔开
    for (int i = 0; i < this->EmpNum; i++)
    {
...
void workerManager ::init_Emp() // 构造函数判断文件不为空时，初始化职工
{
    ifstream ifs;
    ifs.open(FILE, ios::in);
    int id;
