...
一个基于 C++ **面向对象多态**的控制台职工信息管理系统，支持职工信息的增删改查、排序以及文件持久化存储。
...
Worker（抽象基类）
    │
    ├── Employee  → getDeptName() 返回 "员工"   showInfo() 显示 "完成经理的任务"
    ├── Menager   → getDeptName() 返回 "经理"   showInfo() 显示 "完成老板的任务"
    └── Boss      → getDeptName() 返回 "总裁"   showInfo() 显示 "管理公司所有事物"
```
...
EmpArray[i]->showInfo()
    │
    ├── 实际指向 Employee → 调用 Employee::showInfo()  → "岗位职责：完成经理的任务"
    ├── 实际指向 Menager  → 调用 Menager::showInfo()   → "岗位职责：完成老板的任务"
    └── 实际指向 Boss     → 调用 Boss::showInfo()      → "岗位职责：管理公司所有事物"
```
...
workerManager::workerManager()
{
    ...
    // 情况③：文件存在且有数据
    int num = this->get_EmpNum();      // 先统计职工人数
    this->EmpNum = num;
    this->EmpArray = new Worker *[num]; // 按人数开辟指针数组空间
    this->init_Emp();                  // 逐行读取文件，创建子类对象存入数组
}
```
